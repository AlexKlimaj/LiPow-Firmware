...
void PORTx_IRQHandler(uint8_t PortNum)
{
  ...
  if ((hucpd->IMR & _interrupt) != 0)
  {
    ...
    if (_interrupt & UCPD_SR_TXMSGSENT)
    {
    ...
    if (_interrupt & UCPD_SR_TXMSGABT)
    {
    ...
    if (_interrupt & UCPD_SR_HRSTDISC)
    {
    ...
    /* TXUND : tx underrun detected */
    if (_interrupt & UCPD_SR_HRSTSENT)
    {
    ...
    if (_interrupt & UCPD_SR_RXMSGEND)
    {
      ...
      uint16_t _datasize = hucpd->RX_PAYSZ;
      LL_UCPD_ClearFlag_RxMsgEnd(hucpd);

      // disable DMA
      CLEAR_BIT(Ports[PortNum].hdmarx->CCR, DMA_CCR_EN);
      //      CLEAR_BIT(Ports[PortNum].husbpd->CR, UCPD_CR_PHYRXEN);
    ...
    if (_interrupt & UCPD_SR_TYPECEVT1)
    {
      Ports[PortNum].CC1 = hucpd->SR & UCPD_SR_TYPEC_VSTATE_CC1;
      Ports[PortNum].CC2 = hucpd->SR & UCPD_SR_TYPEC_VSTATE_CC2;
    ...
    /* check TYPECEVT1IE/TYPECEVT1IE */
    if (_interrupt & UCPD_SR_TYPECEVT2)
    {
      Ports[PortNum].CC1 = hucpd->SR & UCPD_SR_TYPEC_VSTATE_CC1;
      Ports[PortNum].CC2 = hucpd->SR & UCPD_SR_TYPEC_VSTATE_CC2;
